(function() {
  ...
  // Load the static background image automatically
  const EMBED_BG = "background.png"; // this file must exist in repo root
  bg.style.backgroundImage = "url('" + EMBED_BG + "')";
  if (badge) badge.textContent = "Mapa cargado ✓";
    ...
    parts.forEach(p => {
      ...
      if (street && bare) {
        ...
        span.addEventListener('click', function() {
          ...
          if (debug) debug.textContent = "Última palabra: " + bare;
          drawStreets();
        });
  ...
  if (testBtn) {
    testBtn.addEventListener('click', function() {
      ...
      if (badge) badge.textContent = "Probando resaltado…";
      drawStreets();
      setTimeout(function() {
        hover = prevHover; active = prevActive; 
        if (badge) badge.textContent = "Mapa cargado ✓";
        drawStreets();
      }, 1500);
    });
